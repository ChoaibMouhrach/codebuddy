...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ isLoading, disabled, children, ...rest }, ref) => {
  return (
    <ButtonComp {...rest} disabled={isLoading || disabled} ref={ref} >
      {isLoading ? <> <RiLoader4Line className="animate-spin mr-2" /> {children}</> : children}
    </ButtonComp>
  )
});
